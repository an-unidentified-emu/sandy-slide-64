...
void bhv_hidden_green_coin_star_init(void) {
    ...
    if (o->oBehParams2ndByte != 0) {
        ...
        s16 numGreenCoinsRemaining = count_objects_with_behavior(bhvGreenCoin);
        if (numGreenCoinsRemaining == 0) {
            starObj = spawn_object_abs_with_rot(o, 0, MODEL_STAR, bhvStar, o->oPosX, o->oPosY, o->oPosZ, 0, 0, 0);
            starObj->oBehParams = o->oBehParams;
            o->activeFlags = ACTIVE_FLAG_DEACTIVATED;
        }
        o->oHiddenStarTriggerTotal = numGreenCoinsRemaining + gGreenCoinsCollected;
        o->oHiddenStarTriggerCounter = o->oHiddenStarTriggerTotal - numGreenCoinsRemaining;
    }
